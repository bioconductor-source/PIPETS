...
the prefix for the output file name `OutputFileID`, and the type of input data
`inputDataFormat`.
...
dir <- system.file("extdata", package="PIPETS")
samples <- read.table(file.path(dir,"PIPETS_TestData.bed"), header=FALSE)

#Bed File Input
PIPETS_FullRun(inputData = file.path(dir,"PIPETS_TestData.bed"),readLength = 58, 
               OutputFileID = "ExampleResultsRun", inputDataFormat = "bedFile")
...
GRanges_Object <- GRanges(seqnames = samples$V1,
                    ranges = IRanges(start = samples$V2,end = samples$V3),
                    strand = samples$V6)
