...
## Basic PIPETS Run
Here we show the basic walkthrough of running PIPETS on an input Bed file. The
only two parameters that need to be specified to run PIPETS are `inputBedFile`
and the expected `readLength` from the experiment. Running the below
code will prompt you to give a name for the output files, and PIPETS will run
all of the analysis from there.

```{r echo=TRUE, message=FALSE, eval=FALSE}
PIPETS_FullRun(inputBedFile = "~/path/to/bed/file", readLength = X)
```
